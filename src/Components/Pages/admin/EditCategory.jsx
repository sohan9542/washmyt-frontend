...
const EditCategory = () => {
  ...
  const update = () => {
    ...
    let config = {
      method: "put",
      url: `${URI}/api/v1/admin/category/${catID}`,
      headers: {
  ...
  useEffect(() => {
    ...
    axios.request(config)
      .then((response) => {
        setName(response.data?.category?.title)
        let dataArray= response.data?.category?.subcategory
